...
int gameEndCheck(const Settings& settings, const Turn& turn, const Board& board, std::vector<Move> moves) {
    int black_counter = 0;
    int white_counter = 0;
    int man_counter = 0;
    int king_counter = 0;
    const MainTile* tile;
    std::vector<sf::Vector2u> black_pieces_cords;
    std::vector<sf::Vector2u> white_pieces_cords;
    ...
    MovePlanner black_tester(settings, black_turn, board);
    MovePlanner white_tester(settings, white_turn, board);
    bool black_cant_move = true;
    bool white_cant_move = true;
    bool draw = true;
    for (unsigned int i = 1; i <= settings.getBoardSize(); i++) {
        for (unsigned int j = 1; j <= settings.getBoardSize(); j++) {
            ...
            if (tile->hasPiece()) {
                black_counter += tile->getPiece()->getTeam() == Team::Black;
                white_counter += tile->getPiece()->getTeam() == Team::White;
                ...
                if (tile->getPiece()->getTeam() == Team::Black) {
                    black_pieces_cords.push_back(sf::Vector2u(i, j));
                }
                else {
                    white_pieces_cords.push_back(sf::Vector2u(i, j));
                }
            }
        }
    }
    ...
    if (white_counter == 0) {
        std::cout << "black1" << std::endl;
        return BlackWon;
    }
    if (man_counter == 0 && king_counter > 0) {
        std::cout << "hhrrlllllooo" << std::endl;
        for (sf::Vector2u from : black_pieces_cords) {
            ...
            for (int x : range) {
                for (int y : { -x, x }) {
                    ...
                    std::cout << "a" << std::endl;
                    if (black_tester.tryAdding(from, to) == 1 && black_tester.getMoves().at(0).type() == Move::Capture) {
                        std::cout << "can move FROM: X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                        draw = false;
                    }
                }
            }
        }
        for (sf::Vector2u from : white_pieces_cords) {
            ...
            std::cout << "41111" << std::endl;
            for (int x : range) {
                for (int y : { -x, x }) {
                    white_tester.clear();
                    std::cout << "eee" << std::endl;
                    to = sf::Vector2u(sf::Vector2i(from) + sf::Vector2i(x, y));
                    if (white_tester.tryAdding(from, to) == 1 && white_tester.getMoves().at(0).type() == Move::Capture) {
                        std::cout << "can move FROM: X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                        draw = false;
                    }
                }
    ...
    for (sf::Vector2u from : black_pieces_cords) {
        ...
        for (int x : range) {
            for (int y : { -x, x }) {
                black_tester.clear();
                to = sf::Vector2u(sf::Vector2i(from) + sf::Vector2i(x, y));
                if (black_tester.tryAdding(from, to) == 1) {
                    //std::cout << "can move FROM: X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                    black_cant_move = false;
                }
            }
        }
    }
    for (sf::Vector2u from : white_pieces_cords) {
        ...
        for (int x : range) {
            for (int y : { -x, x }) {
                white_tester.clear();
                to = sf::Vector2u(sf::Vector2i(from) + sf::Vector2i(x, y));
                if (white_tester.tryAdding(from, to) == 1) {
                    //std::cout << "can move FROM: X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                    white_cant_move = false;
                }
            }
        }
    }
    ...
    for (sf::Vector2u from : pieces_cords) {
        ...
        for (int x : range) {
            for (int y : { -x , x }) {
                ...
                to = sf::Vector2u(sf::Vector2i(from) + sf::Vector2i(x, y));
                if (board.at(from)->getPiece()->getType() == Piece::Man) {
                    //std::cout << "MAN from X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                }
                else if (board.at(from)->getPiece()->getType() == Piece::King) {
                    //std::cout << "KING from X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
                }
                if (tester.tryAdding(from, to) == 1 && tester.getMoves().at(0).type() == Move::Capture) {
                    //std::cout << "missed capture FROM: X=" << from.x << " Y=" << from.y << " TO: X=" << to.x << " Y=" << to.y << std::endl;
            ...
            if (!game_started) {
                ...
                if (confirm_move_button.onClick(window, event)) {
                    if (move.size() > 0) {
                        ...
                        //std::cout << "missed captures=" << missed_captures.size() << std::endl;
                        ...
                        std::cout << (bool)turn->getTeam() << " game state=" << gameEndCheck(*settings, *turn, *board, move) << std::endl;
